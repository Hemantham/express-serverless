...
exports.encryptLoginCredentials = void 0;
const crypto_1 = require("crypto");
const crypto = crypto_1.webcrypto;
function encryptLoginCredentials(accessID, password, pubKey) {
    ...
    if (2147483647 > length2) {
        ...
        var val = applySymmetricEncryption(buffer1, buffer2, encodedBuffer).then(function (sumenc) {
            ...
            var base64BufferNew = new Uint8Array(keyBase64Buffer.length + ivBase64Buffer.length);
            var bufferKey = WriteBase64(base64BufferNew, key, 0, 16);
            var bufferVi = WriteBase64(base64BufferNew, iv, 16, 16);
            var encryptval = applyAsymmetricEncryption(publicKeyBase64Buffer, base64BufferNew).then(function (encrypted) {
                ...
                WriteBase64(arrBuffer, payload, contentlen + 4, payloadLength - (contentlen + 4));
...
exports.encryptLoginCredentials = encryptLoginCredentials;
...
function applyAsymmetricEncryption(publicKeyBuffer, payloadBuffer) {
    return Getsubtle()
        .importKey("spki", publicKeyBuffer, { name: "RSA-OAEP", hash: { name: "SHA-256" } }, !1, ["encrypt"])
        .then(function (publicKeyBuffer) {
        return Getsubtle().encrypt({ name: "RSA-OAEP", hash: { name: "SHA-256" } }, publicKeyBuffer, payloadBuffer);
    });
}
function Base64Buffer(buffer) {
    var base64Encoded = atob(buffer);
    for (var t = new Uint8Array(base64Encoded.length), n = 0; n < base64Encoded.length; n++) {
...
function WriteBase64(base64Buffer, data, startIndex, lengthOfData) {
    ...
    if (((data = data.replace(ee, "")), data.length < 2))
        return "";
    for (; data.length % 4 !== 0;)
        data += "=";
    ...
    var n = startIndex;
    var r = lengthOfData;
    for (var o = 0; o < r && !(o + n >= base64Buffer.length || o >= dataByteArray.length); ++o)
        base64Buffer[o + n] = dataByteArray[o];
    return base64Buffer;
}
function ToByteArray(e) {
    for (var l = [], f = [], d = "undefined" !== typeof Uint8Array ? Uint8Array : Array, p = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", h = 0, v = p.length; h < v; ++h) {
        (l[h] = p[h]), (f[p.charCodeAt(h)] = h);
    }
    f["-".charCodeAt(0)] = 62;
    f["_".charCodeAt(0)] = 63;
    for (var t, n = ValidateLengthOf4(e), o = n[0], a = n[1], u = new d(factorIn(o, a)), c = 0, s = a > 0 ? o - 4 : o, l = 0; l < s; l += 4)
        (t =
            (f[e.charCodeAt(l)] << 18) |
...
function WriteBuffer(e, t, buffer3) {
    ...
    for (var o = 0; o < n && !(o + t >= buffer3.length || o >= newBuffer.length); ++o) {
...
function applySymmetricEncryption(bufffer1, buffer2, utf8Buffer) {
    return Getsubtle()
        ...
        .then(function (key) {
        return Getsubtle().encrypt({ name: "AES-CBC", iv: buffer2 }, key, utf8Buffer);
    });
}
function Getsubtle() {
